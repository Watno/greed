package spacealert.core.threats;

import com.fasterxml.jackson.annotation.JsonProperty;

import java.util.*;

public class Trajectory {
    @JsonProperty
    private final String name;
    @JsonProperty
    private final Map<Integer, Action> actionSpaces;
    @JsonProperty
    private final int length;

    private Trajectory(int length, int positionOfX, List<Integer> positionsOfY, String name) {
        this.name = name;
        actionSpaces = new HashMap<>();
        actionSpaces.put(1, Action.Z);
        actionSpaces.put(positionOfX, Action.X);
        for (var positionOfY : positionsOfY) {
            actionSpaces.put(positionOfY, Action.Y);
        }
        this.length = length;
    }

    public static List<Trajectory> all() {
        var result = new ArrayList<>(List.of(
                Trajectory.T1(),
                Trajectory.T2(),
                Trajectory.T3(),
                Trajectory.T4(),
                Trajectory.T5(),
                Trajectory.T6(),
                Trajectory.T7()
        ));
        Collections.shuffle(result);

        return result;
    }

    public static Trajectory T1() {
        return new Trajectory(10, 5, List.of(), "T1");
    }

    public static Trajectory T2() {
        return new Trajectory(11, 8, List.of(), "T2");
    }

    public static Trajectory T3() {
        return new Trajectory(12, 8, List.of(3), "T3");
    }

    public static Trajectory T4() {
        return new Trajectory(13, 9, List.of(5), "T4");
    }

    public static Trajectory T5() {
        return new Trajectory(14, 11, List.of(7), "T5");
    }

    public static Trajectory T6() {
        return new Trajectory(15, 10, List.of(3, 7), "T6");
    }

    public static Trajectory T7() {
        return new Trajectory(16, 12, List.of(5, 8), "T7");
    }

    public List<Action> getActionsBetween(int initialPosition, int targetPosition) {
        var actions = new ArrayList<Action>();
        for (int position = initialPosition - 1; position >= targetPosition; position--) {
            if (actionSpaces.containsKey(position)) {
                actions.add(actionSpaces.get(position));
            }
        }
        return actions;
    }

    public int getStartingPosition() {
        return length + 1;
    }
}
